...
from kari_core.core import util as core_util
from kari_core.core.util import (
    _exif_cache_key,
    _exif_cache_put,
    clear_exif_cache,
    exif_cache_size,
...
class TestGetExifCached:
    def test_first_call_invokes_exiftool_then_caches(self, fake_jpeg):
        ...
        with patch("kari_core.core.util.subprocess.check_output", return_value=fake_output) as mock_exif:
    ...
    def test_failure_returns_empty_and_does_not_cache(self, fake_jpeg):
        with patch(
            "kari_core.core.util.subprocess.check_output", side_effect=RuntimeError("boom")
        ):
    ...
    def test_mtime_change_invalidates_cache(self, fake_jpeg):
        ...
        with patch("kari_core.core.util.subprocess.check_output", side_effect=fake_check):
            r1 = get_exif(str(fake_jpeg))
...
class TestGetExifBatchCached:
    def test_all_hits_skip_exiftool(self, tmp_path):
        ...
        with patch("kari_core.core.util.subprocess.check_output") as mock_exif:
    ...
    def test_partial_miss_triggers_exiftool_for_misses_only(self, tmp_path):
        ...
        with patch(
            "kari_core.core.util.subprocess.check_output", return_value=fake_output
        ) as mock_exif:
            r = get_exif_batch([str(a), str(b)])
    ...
    def test_writes_back_misses_into_cache(self, tmp_path):
        ...
        with patch("kari_core.core.util.subprocess.check_output", return_value=fake_output):
