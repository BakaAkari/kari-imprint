...
from kari_core.processor.perf import (
...
def _worker_process_one(task: BatchTask) -> BatchResultItem:
    ...
    - 显式 ``import kari_core.processor`` 触发处理器装饰器注册（每个 worker 进程独立一份注册表）；
    ...
    try:
        ...
        import kari_core.processor  # noqa: F401
...
def build_tasks(
    ...
    *,
    ...
    if pre_loaded_exif_map is None:
        # 延迟导入避免循环依赖
        from kari_core.core.util import get_exif_batch
